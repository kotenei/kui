...
class ProgressCircle extends Component {
    ...
    render() {
        ...
        let fontSize=this.getFontSize();
        return (
            <div className={`${prefixCls}-circle`} style={{ width, height: width, fontSize }}>
                <svg viewBox="0 0 100 100">
                    <path className={`${prefixCls}-circle-track`} d={d} stroke="#e5e9f2" fill="none" strokeWidth={strokeWidth}></path>
                    <path className={`${prefixCls}-circle-path`} d={d} strokeLinecap="round" fill="none" strokeWidth={strokeWidth} style={circlePathStyle} stroke={stroke}></path>
                </svg >
